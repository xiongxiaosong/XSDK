...
public class WheelView extends View {
	...
	/** 左右内边距2DP */
	private static final int PADDING = TransformUtil.dip2px(2);
	/** 默认可见Item数5 */
	private static final int DEF_VISIBLE_ITEMS = 5;
	/** 文字默认颜色 */
	private static final int DEF_TEXT_COLOR = 0xFF101010;
	/** 默认文字字体大小 */
	private static final int DEF_TEXT_SIZE = TransformUtil.dip2px(15);
	/** 当前Item */
	...
	private void setCustomAttributes(AttributeSet attrs) {
		TypedArray a = mContext.obtainStyledAttributes(attrs,
				R.styleable.XWheelView);
		int n = a.getIndexCount();
		for (int i = 0; i < n; i++) {
			int attr = a.getIndex(i);
			if(attr==R.styleable.XWheelView_xwheeliscircle){
				isCyclic = a.getBoolean(attr, false);
			}else if(attr==R.styleable.XWheelView_xwheeltextcolor){
				textcolor = a.getColor(attr, DEF_TEXT_COLOR);
			}else if(attr==R.styleable.XWheelView_xwheeltextsize){
				textsize = a.getDimensionPixelSize(attr, DEF_TEXT_SIZE);
			}else if(attr==R.styleable.XWheelView_xwheelvisibleitems){
				visibleItems = a.getInt(attr, DEF_VISIBLE_ITEMS);
			}
		}
		a.recycle();
	}
