...
public class ChineseToPinyin {
	...
	private static ChineseToPinyin characterParser = new ChineseToPinyin();
	 ...
	 * @return 得到的拼音字符串
	 */
	public String oneChineseToPinyin(String str) {
		String result = null;
	 ...
	 * @return 得到的拼音字符串
	 */
	public String moreChineseToPinyin(String chs) {
		String key, value;
		buffer = new StringBuilder();
		for (int i = 0; i < chs.length(); i++) {
			key = chs.substring(i, i + 1);
			if (key.getBytes().length >= 2) {
				value = (String) oneChineseToPinyin(key);
				if (value == null) {
					value = "unknown";
				}
			} else {
